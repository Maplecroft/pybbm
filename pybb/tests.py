...
from pybb.models import Topic, TopicReadTracker, Forum, ForumReadTracker, Post, Category, PollAnswer, Profile, Client as ClientModel
...
class SharedTestModule(object):
    ...
    def create_initial(self, post=True):
        self.client_for = ClientModel.objects.create(name='Pybb', code='pybb')
        self.category = Category.objects.create(name='foo', client=self.client_for)
...
class FeaturesTest(TestCase, SharedTestModule):
    ...
    def test_profile_edit(self):
        # Self profile edit
        self.login_client()
        response = self.client.get(reverse('pybb_pybb:edit_profile'))
        self.assertEqual(response.status_code, 200)
        values = self.get_form_values(response, 'profile-edit')
        values['signature'] = 'test signature'
        response = self.client.post(reverse('pybb_pybb:edit_profile'), data=values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_pagination_and_topic_addition(self):
        for i in range(0, defaults.PYBB_FORUM_PAGE_SIZE + 3):
            ...
            topic.save()
        url = reverse('pybb_pybb:forum', args=[self.forum.id])
        response = self.client.get(url)
    ...
    def test_topic_addition(self):
        self.login_client()
        add_topic_url = reverse('pybb_pybb:add_topic', kwargs={'forum_id': self.forum.id})
    ...
    def test_read_tracking(self):
        ...
        # Forum status
        tree = html.fromstring(client.get(reverse('pybb_pybb:index')).content)
        self.assertTrue(
        ...
        self.assertFalse(
            tree.xpath('//a[@href="%s"]/parent::td[contains(@class,"unread")]' % topic.forum.get_absolute_url()))
        # Post message
        add_post_url = reverse('pybb_pybb:add_post', kwargs={'topic_id': topic.id})
        ...
        post = Post(topic=topic, user=self.user, body='one')
        post.save()
        client.get(reverse('pybb_pybb:mark_all_as_read'))
        tree = html.fromstring(client.get(reverse('pybb_pybb:index')).content)
        self.assertFalse(
            tree.xpath('//a[@href="%s"]/parent::td[contains(@class,"unread")]' % topic.forum.get_absolute_url()))
        # Empty forum - readed
    ...
    def test_read_tracking_multi_user(self):
        ...
        add_post_url = reverse('pybb_pybb:add_post', kwargs={'topic_id': topic_1.id})
    ...
    def test_read_tracker_after_posting(self):
        client = Client()
        client.login(username='zeus', password='zeus')
        add_post_url = reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id})
        response = client.get(add_post_url)
        values = self.get_form_values(response)
        values['body'] = 'test tracking'
        response = client.post(add_post_url, values, follow=True)
    ...
    def test_read_tracker_when_topics_forum_changed(self):
        ...
        # read all
        client_ann.get(reverse('pybb_pybb:mark_all_as_read'))
        self.assertListEqual(
            [t.unread for t in pybb_topic_unread([topic_1, topic_2], user_ann)],
            [False, False])
        self.assertListEqual(
    ...
    def test_open_first_unread_post(self):
        ...
        response = client_ann.get(reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), data={'first-unread': 1},
                                  follow=True)
        self.assertRedirects(response,
                             '%s?page=%d#post-%d' % (reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), 1, post_1_1.id))
        response = client_ann.get(reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), data={'first-unread': 1},
                                  follow=True)
        self.assertRedirects(response,
                             '%s?page=%d#post-%d' % (reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), 1, post_1_2.id))

        response = client_ann.get(reverse('pybb_pybb:topic', kwargs={'pk': topic_2.id}), data={'first-unread': 1},
                                  follow=True)
        self.assertRedirects(response,
                             '%s?page=%d#post-%d' % (reverse('pybb_pybb:topic', kwargs={'pk': topic_2.id}), 1, post_2_1.id))
        ...
        response = client_ann.get(reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), data={'first-unread': 1},
                                  follow=True)
        self.assertRedirects(response,
                             '%s?page=%d#post-%d' % (reverse('pybb_pybb:topic', kwargs={'pk': topic_1.id}), 1, post_1_3.id))
    ...
    def test_latest_topics(self):
        ...
        client_for = ClientModel.objects.create(name='Pybb', code='pybb')
        category_2 = Category.objects.create(name='cat2', client=client_for)
        ...
        self.login_client()
        response = self.client.get(reverse('pybb_pybb:topic_latest'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_hidden(self):
        client = Client()
        client_for = ClientModel.objects.create(name='Pybb', code='pybb')
        category = Category(name='hcat', hidden=True, client=client_for)
        category.save()
        ...
        self.assertFalse(category.id in [c.id for c in client.get(reverse('pybb_pybb:index')).context['categories']])
        ...
        self.assertNotContains(client.get(reverse('pybb_pybb:index')), forum_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('pybb_pybb:feed_topics')), topic_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('pybb_pybb:feed_topics')), topic_in_hidden.get_absolute_url())

        self.assertNotContains(client.get(reverse('pybb_pybb:feed_posts')), post_hidden.get_absolute_url())
        self.assertNotContains(client.get(reverse('pybb_pybb:feed_posts')), post_in_hidden.get_absolute_url())
        ...
        self.user.save()
        self.assertTrue(category.id in [c.id for c in client.get(reverse('pybb_pybb:index')).context['categories']])
        self.assertContains(client.get(reverse('pybb_pybb:index')), forum_hidden.get_absolute_url())
    ...
    def test_inactive(self):
        self.login_client()
        url = reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id})
        response = self.client.get(url)
        values = self.get_form_values(response)
    ...
    def test_csrf(self):
        ...
        client.login(username='zeus', password='zeus')
        post_url = reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id})
        ...
        values = self.get_form_values(response)
        response = client.post(reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id}), values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_blocking(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.get(reverse('pybb_pybb:block_user', args=[user.username]), follow=True)
        self.assertEqual(response.status_code, 405)
        response = self.client.post(reverse('pybb_pybb:block_user', args=[user.username]), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        user.is_active = True
        user.save()
        response = self.client.post(reverse('pybb_pybb:block_user', args=[user.username]),
    ...
    def test_user_unblocking(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.get(reverse('pybb_pybb:unblock_user', args=[user.username]), follow=True)
        self.assertEqual(response.status_code, 405)
        response = self.client.post(reverse('pybb_pybb:unblock_user', args=[user.username]), follow=True)
        self.assertEqual(response.status_code, 200)
        user = User.objects.get(username=user.username)
    ...
    def test_ajax_preview(self):
        self.login_client()
        response = self.client.post(reverse('pybb_pybb:post_ajax_preview'), data={'data': '[b]test bbcode ajax preview[b]'})
    ...
    def test_quote(self):
        self.login_client()
        response = self.client.get(reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id}),
    ...
    def test_edit_post(self):
        self.login_client()
        edit_post_url = reverse('pybb_pybb:edit_post', kwargs={'pk': self.post.id})
    ...
    def test_admin_post_add(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.post(reverse('pybb_pybb:add_post', kwargs={'topic_id': self.topic.id}),
    ...
    def test_stick(self):
        ...
        self.login_client()
        self.assertEqual(
            self.client.get(reverse('pybb_pybb:stick_topic', kwargs={'pk': self.topic.id}), follow=True).status_code, 200)
        self.assertEqual(
            self.client.get(reverse('pybb_pybb:unstick_topic', kwargs={'pk': self.topic.id}), follow=True).status_code, 200)
    ...
    def test_delete_view(self):
        ...
        self.user.save()
        self.login_client()
        response = self.client.post(reverse('pybb_pybb:delete_post', args=[post.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        # Delete topic
        response = self.client.post(reverse('pybb_pybb:delete_post', args=[self.post.id]), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_open_close(self):
        ...
        self.user.save()
        self.login_client()
        add_post_url = reverse('pybb_pybb:add_post', args=[self.topic.id])
        response = self.client.get(add_post_url)
        values = self.get_form_values(response)
        values['body'] = 'test closed'
        response = self.client.get(reverse('pybb_pybb:close_topic', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        response = self.client.post(add_post_url, values, follow=True)
        self.assertEqual(response.status_code, 403)
        response = self.client.get(reverse('pybb_pybb:open_topic', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        response = self.client.post(add_post_url, values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_subscription(self):
        ...
        response = client.get(reverse('pybb_pybb:add_subscription', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        client.login(username='user2', password='user2')
        self.assertTrue([msg for msg in mail.outbox if new_post.get_absolute_url() in msg.body])
        response = client.get(reverse('pybb_pybb:delete_subscription', args=[self.topic.id]), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_views(self):
        response = self.client.get(reverse('pybb_pybb:user', kwargs={'username': self.user.username}))
        self.assertEqual(response.status_code, 200)

        response = self.client.get(reverse('pybb_pybb:user_posts', kwargs={'username': self.user.username}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['object_list'].count(), 1)

        response = self.client.get(reverse('pybb_pybb:user_topics', kwargs={'username': self.user.username}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['object_list'].count(), 1)
...
class PreModerationTest(TestCase, SharedTestModule):
    ...
    def test_premoderation(self):
        ...
        post = Post.objects.get(body='test premoderation')
        response = admin_client.get(reverse('pybb_pybb:moderate_post', kwargs={'pk': post.id}), follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        post.save()
        client.login(username='zeus', password='zeus')
        response = client.get(reverse('pybb_pybb:moderate_post', kwargs={'pk': post.id}), follow=True)
        self.assertEqual(response.status_code, 403)
        ...
        response = admin_client.get(reverse('pybb_pybb:moderate_post',
...
class PollTest(TestCase, SharedTestModule):
    ...
    def test_regression_poll_deletion_after_second_post(self):
        ...
        add_post_url = reverse('pybb_pybb:add_post', kwargs={'topic_id': new_topic.id})
        response = self.client.get(add_post_url)
        values = self.get_form_values(response)
    ...
    def test_poll_edit(self):
        edit_topic_url = reverse('pybb_pybb:edit_post', kwargs={'pk': self.post.id})
        self.login_client()
    ...
    def test_poll_voting(self):
        ...
        vote_url = reverse('pybb_pybb:topic_poll_vote', kwargs={'pk': self.topic.id})
        ...
        cancel_vote_url = reverse('pybb_pybb:topic_cancel_poll_vote', kwargs={'pk': self.topic.id})
...
class CustomPermissionHandlerTest(TestCase, SharedTestModule):
    ...
    def setUp(self):
        ...
        client_for = ClientModel.objects.create(name='Pybb', code='pybb')
        c_pub = Category(name='public', client=client_for)
        c_pub.save()
        c_hid = Category(name='private', hidden=True, client=client_for)
        c_hid.save()
...
class LogonRedirectTest(TestCase, SharedTestModule):
    ...
    def setUp(self):
        ...
        self.category = Category(
            name='private', hidden=True, client=self.client_for)
        self.category.save()
